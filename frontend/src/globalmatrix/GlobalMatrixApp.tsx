...
export default function GlobalMatrixApp() {
  ...
  useEffect(() => () => { if (cleanupRef.current) cleanupRef.current(); if (simRef.current) clearTimeout(simRef.current); }, []);
      ...
      <div className="telemetry-root" style={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
            ...
            {/* FIX #6: Tabs are now clearly decorative (no pointer cursor, dimmer) */}
            {['NETWORK', 'TELEMETRY', 'ARCS', 'REGIONS'].map((tab, i) => (
              <span key={tab} style={{
                fontFamily: "'Space Grotesk', sans-serif", fontSize: '0.6rem', letterSpacing: '0.12em',
                color: i === 0 ? '#4cd7f6' : 'rgba(220,226,248,0.15)',
        ...
        <div style={{ flex: 1, display: 'flex', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
            ...
            <div className="te-glass" style={{
              ...
              {/* FIX #4: Button now shows different states */}
              <button
